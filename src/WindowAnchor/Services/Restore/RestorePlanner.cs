...
public static class RestorePlanner
{
    ...
    public static RestorePlan Build(
        ...
        RestoreMode mode) => Build(
            snapshot,
            liveInventory,
            monitorTopology,
            mode,
            AppAdapterRegistry.CreatePlanningDefault(),
            persistentApplications: null);

    internal static RestorePlan Build(
        WorkspaceSnapshot snapshot,
        RestoreLiveInventory liveInventory,
        RestoreMonitorTopology monitorTopology,
        RestoreMode mode,
        AppAdapterRegistry appAdapters,
        IEnumerable<PersistentApplicationIdentity>? persistentApplications = null)
    {
        ...
        IReadOnlySet<long> persistentApplicationWindowHandles =
            PersistentApplicationPolicy.GetProtectedWindowHandles(
                liveWindows,
        ...
        for (int entryIndex = 0; entryIndex < savedEntries.Length; entryIndex++)
        {
            ...
            SavedWindowIdentity savedIdentity = appAdapters.EnrichIdentity(
                entry,
            ...
            // A precise Resume with an exact monitor topology does not touch a window that is
            // already in its target state. Preflight still revalidates its HWND and identity.
            bool skipStablePlacement = policy.RepairOnly ||
                (mode.Kind == RestoreModeKind.Resume && monitorTopology.IsExactMatch);
            bool placementNeeded = selectedMatch is not null &&
                (!skipStablePlacement || PlacementNeedsRepair(
                    selected!,
                    placement,
                    requireExactGeometry: policy.RepairOnly));
            if (placementNeeded)
            {
            ...
            else if (selectedMatch is not null)
            {
                ...
                explanation = !placementNeeded
                    ? "The existing window already matches the requested placement; no action is needed."
                    : matchResolution.Explanation;
            }
        ...
        bool requireExactGeometry)
    {
        if (current.ShowCmd != target.ShowCmd ||
            ...
            (requireExactGeometry && target.Strategy != RestorePlacementStrategy.ExactPixels))
        {
            return true;
        }
