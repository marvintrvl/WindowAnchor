        ...
        string? effectiveExecutablePath = null)
    {
        ...
        return runningApplications.Any(application =>
        {
            ...
            if (expectedPath.Length > 0 && observedPath.Length > 0)
            {
                ...
                string expectedFamily = WindowIdentityExtractor.PackageFamilyFromWindowsAppsPath(expectedPath);
                string observedFamily = WindowIdentityExtractor.PackageFamilyFromWindowsAppsPath(observedPath);
                return expectedFamily.Length > 0 &&
                    string.Equals(expectedFamily, observedFamily, StringComparison.OrdinalIgnoreCase) &&
                    string.Equals(
                        System.IO.Path.GetFileName(expectedPath),
                        System.IO.Path.GetFileName(observedPath),
                        StringComparison.OrdinalIgnoreCase) &&
                    expectedProcess.Length > 0 &&
