...
internal sealed class RestorePreflightPhase
{
    ...
    internal RestoreExecutionResult? Execute(RestoreExecutionContext context)
    {
        ...
        foreach (RestorePlanEntry entry in plan.Entries.OrderBy(entry => entry.EntryIndex))
        {
            ...
            IndexedRestoreAction? firstAction = entryActions.Length > 0
                ? entryActions[0]
                : null;
            RestoreEntryExecutionState state = context.Entries[entry.EntryIndex];
            ...
            if (entry.SelectedMatch is not null)
            {
                ...
                {
                    MarkEntryStale(firstAction, state, context.Results, staleWindow.Value);
                    continue;
                }
            }
            ...
            if (firstAction is null && entry.SelectedMatch is not null)
            {
                ...
                state.AssignedWindowHandle = entry.SelectedMatch.WindowHandle;
                state.Explanation =
                    "The revalidated live window already matches the approved target placement.";
                context.AssignedHwnds.Add(handle);
            }
        }
        ...
        if (!context.Results.Values.Any(result =>
            ...
            !context.Entries.Values.Any(state =>
                state.Status == RestoreExecutionEntryStatus.Stale))
        {
            return null;
        }
    ...
    private static void MarkEntryStale(
        IndexedRestoreAction? action,
        ...
        RestorePlanStaleReason reason)
    {
        if (action is { } item)
        {
            RestoreExecutionSupport.MarkStale(item, state, results, reason);
            return;
        }

        RestoreExecutionSupport.MarkEntryStale(state, reason);
    }
}
