...
internal static class WindowMatchResolver
{
    ...
    public static WindowMatchResolution ResolveCandidates(
        ...
        WindowMatchPolicy? policy = null) => WindowMatchResolutionSelector.Resolve(candidates, policy ?? WindowMatchPolicy.Default);
    ...
    public static double TitleSimilarity(string? first, string? second) =>
        WindowMatchTextSimilarity.Calculate(first, second);
        ...
        WindowMatchPolicy policy)
    {
        ...
        bool legacyPackagedIdentityExact = !saved.IsWebApp &&
            string.IsNullOrWhiteSpace(saved.AppUserModelId) &&
            PackageFamilyExact(saved, live) &&
            ProcessExact(saved, live) &&
            string.Equals(
                Path.GetFileName(saved.ExecutablePath),
                Path.GetFileName(live.ExecutablePath),
                StringComparison.OrdinalIgnoreCase);
        if (legacyPackagedIdentityExact)
        {
            Add(evidence, WindowMatchEvidenceKind.PackageFamilyExact, true, 9600,
                "Legacy package family and executable name match across a Store update.");
            return Candidate(saved, live, true, 9600, WindowMatchConfidence.Exact, evidence, null);
        }
