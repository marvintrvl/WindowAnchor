...
internal sealed class RestoreObservationBuilder
{
        ...
        WorkspaceSnapshot snapshot)
    {
        ...
        for (int entryIndex = 0; entryIndex < snapshot.Entries.Count; entryIndex++)
        {
            ...
            {
                observations.Add(ObserveWithLogicalAlias(
                    entryIndex,
                    RestoreResourceKind.LaunchTarget,
                    entry.LaunchArg,
                    entry.LogicalLaunchArg));
            }
    ...
    private RestoreResourceObservation ObserveWithLogicalAlias(
        int entryIndex,
        RestoreResourceKind kind,
        string absoluteTarget,
        string? logicalTarget)
    {
        RestoreResourceObservation exact = _restoreResources.Observe(entryIndex, kind, absoluteTarget);
        if (exact.Availability == RestoreResourceAvailability.Available)
            return exact;

        string? aliasTarget = LogicalPathAliasResolver.Resolve(
            logicalTarget,
            _settingsService?.Settings.LogicalPathAliases);
        if (string.IsNullOrWhiteSpace(aliasTarget) || string.Equals(
                aliasTarget,
                absoluteTarget,
                StringComparison.OrdinalIgnoreCase))
            return exact;

        RestoreResourceObservation aliased = _restoreResources.Observe(entryIndex, kind, aliasTarget);
        return aliased.Availability == RestoreResourceAvailability.Available ? aliased : exact;
    }
